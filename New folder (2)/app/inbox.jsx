      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {threads.map((thread) => (
          <ThemedCard key={thread.key} style={styles.card}>
            ...
            <Spacer height={8} />
            <ThemedText>
                  {thread.otherIdentity?.role === 'company'
                ? 'Compagnie'
                : thread.otherIdentity?.role === 'client'
                  ? 'Client'
                  : 'Interlocuteur'}
              {' : '}
              {thread.otherIdentity?.label || thread.otherParty?.email || thread.otherAccountId || 'Utilisateur'}
            </ThemedText>
                        {thread.otherIdentity?.detail && thread.otherIdentity.detail !== thread.otherIdentity.label && (
              <ThemedText>Email : {thread.otherIdentity.detail}</ThemedText>
            )}
            ...
            <ThemedButton
              onPress={() =>
                router.push({
                  ...
                  params: {
                    offerId: thread.offer?.id || thread.offerId || thread.lastMessage?.insurance_offer_id,
