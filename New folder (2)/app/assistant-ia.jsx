...
import { detectClientProfile, buildPreferenceConsistencyAlert } from '../lib/ai'
...
function AssistantIAContent() {
  ...
  const [preferenceSnapshot, setPreferenceSnapshot] = useState(null)
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setPreferenceSnapshot(preference)
  ...
  const clientProfileInsight = detectClientProfile({
    profile,
    preference: {
      ...preferenceSnapshot,
  ...
  const consistencyAlerts = buildPreferenceConsistencyAlert({
    ...
    <ThemedView safe style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <ThemedText style={styles.subtitle}>
          Conseiller digital, detection de profil et controle de coherence
        </ThemedText>

        <Spacer height={20} />

        <ThemedCard style={styles.card}>
          <ThemedText title style={styles.cardTitle}>Profil detecte</ThemedText>
          <Spacer height={8} />
          <ThemedText>{clientProfileInsight.summary}</ThemedText>
        </ThemedCard>

        {consistencyAlerts.length > 0 && (
          <ThemedCard style={styles.card}>
            <ThemedText title style={styles.cardTitle}>Alertes de coherence</ThemedText>
            <Spacer height={8} />
            {consistencyAlerts.map((alert, index) => (
              <ThemedText key={`alert-${index}`}>- {alert}</ThemedText>
            ))}
          </ThemedCard>
        )}
