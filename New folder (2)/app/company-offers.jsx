...
import { buildCompanyOfferInsights } from '../lib/ai'
...
function CompanyOffersContent() {
  ...
  const selectedType = insuranceTypes.find((type) => type.id === selectedTypeId)
  const offerInsights = useMemo(
    () => buildCompanyOfferInsights({ offerForm, insuranceTypeName: selectedType?.nom }),
    [offerForm, selectedType?.nom]
  )
  ...
  const handleCreateOffer = async () => {
    ...
    if (
      ...
      setFeedback({
        type: 'error',
        message:
           "Veuillez renseigner le type, le nom de l'assurance, la description, le public concerne, les avantages, les tarifs TTC et les modes de contact.",
      })
      return
    }
    ...
    <ThemedView safe style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <ThemedText style={styles.subtitle}>
          IA d'assistance compagnie pour mieux publier et mieux presenter vos offres
        </ThemedText>
        ...
        <ThemedCard style={styles.card}>
          <ThemedText title style={styles.cardTitle}>
            Assistant IA compagnie
          </ThemedText>
          <Spacer height={12} />
          {offerInsights.warnings.length ? (
            <>
              <ThemedText title style={styles.cardSubTitle}>Points a completer</ThemedText>
              {offerInsights.warnings.map((warning, index) => (
                <ThemedText key={`warn-${index}`}>- {warning}</ThemedText>
              ))}
            </>
          ) : (
            <ThemedText>Toutes les informations essentielles semblent presentes.</ThemedText>
          )}

          <Spacer height={8} />

          {offerInsights.suggestions.length > 0 && (
            <>
              <ThemedText title style={styles.cardSubTitle}>Suggestions IA</ThemedText>
              {offerInsights.suggestions.map((suggestion, index) => (
                <ThemedText key={`sg-${index}`}>- {suggestion}</ThemedText>
              ))}
            </>
          )}
        </ThemedCard>
        ...
        <ThemedCard style={styles.card}>
          ...
          <ThemedTextInput
            style={[styles.input, styles.textArea]}
            placeholder="Public concerne par cette offre"
            multiline
          ...
          <ThemedTextInput
            style={[styles.input, styles.textArea]}
            placeholder="Modes de contact proposes (messagerie, telephone, agence...)"
            multiline
        ...
        {offers.map((offer) => (
          <ThemedCard key={offer.id} style={styles.card}>
            ...
            <Spacer height={8} />
            <ThemedText>Public concerne : {offer.who_can_subscribe || 'Non renseigne'}</ThemedText>
            ...
            <ThemedText>Modes de contact : {offer.payment_methods || 'Non renseignes'}</ThemedText>
