export function detectClientProfile({ profile, preference, subscriptions = [] }) {
  ...
  if (historyCount > 0) {
    reasons.push("historique d'interet deja present dans l'application")
  }
...
export function buildCompanyOfferInsights({ offerForm, insuranceTypeName }) {
  ...
  if (!String(offerForm.paymentMethods ?? '').trim()) {
    warnings.push('indiquer au moins un mode de paiement')
  }

  if (!String(offerForm.whoCanSubscribe ?? '').trim()) {
    warnings.push('preciser quel public peut souscrire')
  }
