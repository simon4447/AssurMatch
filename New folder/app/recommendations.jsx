...
import { fetchInsuranceTypes, fetchOfferRecommendations, fetchClientSubscriptions } from '../lib/insurance'
import { fetchLatestClientPreference } from '../lib/profile'
import { compareOffers, detectClientProfile, summarizeOffer } from '../lib/ai'
...
function RecommendationsContent() {
  ...
  useEffect(() => {
    const loadRecommendations = async () => {
      ...
      try {
        const [types, preference, subscriptions] = await Promise.all([
          fetchInsuranceTypes(),
          fetchLatestClientPreference(profile.id),
          fetchClientSubscriptions(profile.id),
        ])
        ...
        setComparisonNotes(compareOffers(topOffers))
        setProfileInsight(
          detectClientProfile({
            profile,
            preference: {
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {profileInsight && (
          <ThemedCard style={styles.card}>
            <ThemedText title style={styles.cardTitle}>Lecture IA du profil</ThemedText>
            <Spacer height={8} />
            <ThemedText>{profileInsight.summary}</ThemedText>
          </ThemedCard>
        )}

        {comparisonNotes.length > 0 && (
          <ThemedCard style={styles.card}>
            <ThemedText title style={styles.cardTitle}>Comparaison intelligente</ThemedText>
            <Spacer height={8} />
            {comparisonNotes.map((note, index) => (
              <ThemedText key={`cmp-${index}`}>- {note}</ThemedText>
            ))}
          </ThemedCard>
        )}
